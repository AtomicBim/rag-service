...
-   **`rag-embedding`**: Создает векторные представления (эмбеддинги) текста с использованием API OpenAI.
...
│                                                                         │
│  ┌──────────────┐   HTTP   ┌───────────────────┐   HTTP   ┌───────────┐  │
│  │   rag-chat   │───► 1. ───┤ rag-embedding │───► 2. ───┤ OpenAI    │  │
...
1.  **Векторизация вопроса**: `rag-chat` отправляет текст вопроса в `rag-embedding`.
2.  **Создание эмбеддинга**: `rag-embedding` обращается к API OpenAI для получения векторного представления вопроса.
...
-   **Документация `rag-embedding`:** [http://localhost:8001/docs](http://localhost:8001/docs)
